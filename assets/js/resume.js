...
window.setTimeout(function() {
  ...
  db.collection('users').doc(user.uid).get().then(doc => {
    ...
    if(!doc.data().FirstName) {
      ...
      html +=  `<h4>姓名：`+doc.data().LastName + doc.data().FirstName +`</h4><hr>`;
      html +=  `<h4>性別：`+doc.data().Gender+`</h4><hr>`;
      html +=  `<h4>身份：`+doc.data().Status+`</h4><hr>`;
      html +=  `<h4>地區：`+doc.data().Location[0] +  ` ` + doc.data().Location[1] +`</h4><hr>`;
      html +=  `<h4>對象：`+doc.data().TargetString+`</h4><hr>`;
